...
summary(wine_null)
```
# Full model with every variable and intercation included  
...
It turns out that none of the 2-way interactions increase model fit (does not lower AIC or BIC), so keep only the main effects.  
...
The strongest predictor is the alcohol, while volatile acidity is the second strongest (negative), and the therd strongest is pH (negative).
```{r}
